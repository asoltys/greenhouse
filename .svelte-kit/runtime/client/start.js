...
function create_client({ target, session, base, trailing_slash }) {
  ...
  const stores = {
    ...
    navigating: writable(
      /** @type {import('types').Navigation | null} */ (null)
    ),
  ...
  async function update(intent, redirect_chain, no_cache, opts) {
    ...
    if (navigation_result.redirect) {
      if (
        redirect_chain.length > 10 ||
        redirect_chain.includes(intent.url.pathname)
      ) {
        navigation_result = await load_root_error_page({
          status: 500,
        ...
        if (router_enabled) {
          ...
          await native_navigation(
            new URL(navigation_result.redirect, location.href)
          );
        }
    ...
    if (opts && opts.details) {
      ...
      history[details.replaceState ? "replaceState" : "pushState"](
        details.state,
        "",
        intent.url
      );
    ...
    if (opts) {
      ...
      if (autoscroll) {
        const deep_linked =
          intent.url.hash && document.getElementById(intent.url.hash.slice(1));
        if (scroll) {
  ...
  async function get_navigation_result_from_branch({
    ...
    const filtered = /** @type {import('./types').BranchNode[] } */ (branch.filter(
      Boolean
    ));
  ...
  async function load_root_error_page({ status, error, url }) {
    ...
    const root_error = await load_node({
      ...
      params,
      stuff:
        (root_layout && root_layout.loaded && root_layout.loaded.stuff) || {},
    });
